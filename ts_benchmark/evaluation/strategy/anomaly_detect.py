...
class AnomalyDetect(Strategy):
    ...
    def execute(self, series_name: str, model_factory: ModelFactory) -> Any:
        ...
        try:
            ...
            predict_labels, another = self.detect(test_data, test_label)
            ...
            # 模型score保存到本地
            pr_score = pd.DataFrame(another, columns=['score'])
            folder_path = './Score/Ours/SWAN' + '/'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            output_file = os.path.join(folder_path, 'test_score.txt')
            np.savetxt(output_file, pr_score, fmt='%f')
    ...
    def detect(self, test_data: pd.DataFrame, test_label: pd.DataFrame):
        raise NotImplementedError
...
class FixedDetectScore(AnomalyDetect):
    ...
    def detect(self, test_data, test_label):
        return self.model.detect_score(test_data, test_label)
...
class FixedDetectLabel(AnomalyDetect):
    ...
    def detect(self, test_data, test_label):
        return self.model.detect_label(test_data, test_label)
