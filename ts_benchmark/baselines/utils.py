...
class SegLoader(object):
    def __init__(self, data, win_size, pre_len, step, mode="train"):
        self.mode = mode
        self.step = step
    ...
    def __len__(self):
        ...
        """
        if self.mode == "train":
            return (self.data.shape[0] - self.win_size - self.pre_len) // self.step + 1
        elif self.mode == "val":
            return (self.data.shape[0] - self.win_size - self.pre_len) // self.step + 1
        elif self.mode == "test":
            return (self.data.shape[0] - self.win_size - self.pre_len) // self.step + 1
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            return np.float32(self.data[index:index + self.win_size]), np.float32(self.test_labels[index:index + self.win_size])
        else:
            return np.float32(self.data[index:index + self.win_size]), np.float32(self.test_labels[index:index + self.win_size])
...
class Anomaly_Prediction_SegLoader(object):
    def __init__(self, data, label, seq_len, pre_len, step, mode="train"):
        self.mode = mode
        self.step = step
    ...
    def __len__(self):
        ...
        """
        if self.mode == "train":
            return (self.data.shape[0] - self.seq_len - self.pre_len) // self.step + 1
        elif self.mode == "val":
            return (self.data.shape[0] - self.seq_len - self.pre_len) // self.step + 1
        elif self.mode == "test":
            return (self.data.shape[0] - self.seq_len - self.pre_len) // self.step + 1
    ...
    def __getitem__(self, index):
        index = index * self.step
        # if index + self.seq_len + self.pre_len >= self.data.shape[0]:
        #     index = self.data.shape[0] - self.seq_len - self.pre_len
        ...
        seq_l = self.label[r_begin:r_end]
        
        # # 单变量
        # seq_x = self.data.iloc[s_begin:s_end, 0]
        # seq_y = self.data.iloc[r_begin:r_end, 0]
        # seq_l = self.label.iloc[r_begin:r_end, 0]

        if len(seq_y) < self.pre_len:
            return None

        # # 单变量
        # seq_x = torch.tensor(seq_x.values, dtype=torch.float32).unsqueeze(-1)
        # seq_y = torch.tensor(seq_y.values, dtype=torch.float32).unsqueeze(-1)
        # seq_l = torch.tensor(seq_l.values, dtype=torch.float32).unsqueeze(-1)

        # 多变量
        seq_x = torch.tensor(seq_x.values, dtype=torch.float32)
        seq_y = torch.tensor(seq_y.values, dtype=torch.float32)
        seq_l = torch.tensor(seq_l.values, dtype=torch.float32)

        return seq_x, seq_y, seq_l


def anomaly_detection_data_provider(data, batch_size, win_size=100, pre_len=100, step=100, mode='train'):
    dataset = SegLoader(data, win_size, pre_len, step, mode)
...
def anomaly_prediction_data_provider(data, label, batch_size, seq_len=100, pre_len=100, step=100, mode='train'):
    dataset = Anomaly_Prediction_SegLoader(data, label, seq_len, pre_len, step, mode)
